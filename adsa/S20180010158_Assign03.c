...
/*
    ...
    Enter lower case names in every input.
    Trie for fast searching and employ for hierarchial structure
    Time Complexity - 
        Insertion - O(wordLength)
        Deleting an employee - O(n), n = number of child of employee
        Lowest Common Boss - O(height of hierarchy)
        Printing all employees - O(n), n = number of employees
*/
struct employ
{
    char name[20];
...
struct queue // for printing the tree
{
    struct employ* emp;
    struct queue* next;
};
...
void insert(char e[],int l1, char b[], int l2, int is_boss_present)
{
    ...
    if (is_boss_present == 0)
    {
        ...
        level_of_boss = trie_end->location->level;
    }
    ...
    new_employ->level = level_of_boss + 1; // adding 1 to level of emp
    ...
    new_employ->boss = trie_end->location; // making new employee, child of boss
...
void DeleteEmployee(char e[], int l1, char n[], int l2)
{
    ...
    else
    {
        ...
        if (emp_end->location->level != neighbour_end->location->level)
        {
        ...
        for (int i=0;i<l1;i++)
        {
            if (i != l1-1)
            {
                ...
                if(temp->next[e[i]-'a']->is_end == 1 || flag == 1) // if a level marks the ending of the prefix
                {
...
void LowestCommonBoss(char emp1[], int l1, char emp2[], int l2)
{
    ...
    else
    {
        ...
        if (emp1_end->location->level > emp2_end->location->level)
        {
            int d = emp1_end->location->level - emp2_end->location->level;
            for(int i=0;i<d;i++)
                boss1 = boss1->boss;
        }
        else if (emp1_end->location->level < emp2_end->location->level)
        {
            int d = emp2_end->location->level - emp1_end->location->level;
            for(int i=0;i<d;i++)
                boss2 = boss2->boss;
        }
        if (boss1 == boss2)
